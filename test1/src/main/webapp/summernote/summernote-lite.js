...
var Bullet = /*#__PURE__*/function () {
  ...
  Bullet_createClass(Bullet, [{
    ...
    value: function wrapList(paras, listName) {
      ...
      var listNode = prevList || dom.insertAfter(dom.create(listName || 'UL'), head); // P to LI
...
var Buttons = /*#__PURE__*/function () {
  ...
  Buttons_createClass(Buttons, [{
	...
	var floatCenter = this.context.memo('button.floatCenter', function () {
          return _this3.button({
            contents: '<i class="fa-solid fa-align-center"></i>',
	...
	var imgModify = this.context.memo('button.imgModify', function () {
          return _this3.button({
            contents: '<i class="fa-solid fa-sliders"></i>',
            tooltip: _this3.lang.image.imgModify,
            click:_this3.context.createInvokeHandler('', 'modify') 
          }).render();
        });
...
var ImageDialog = /*#__PURE__*/function () {
  ...
  ImageDialog_createClass(ImageDialog, [{
    ...
    value: function initialize() {
      ...
      var $container = this.options.dialogsInBody ? this.$body : this.options.container;
	  /*var imgBorder = "<div style='margin:30px 0 30px 0; border:1px solid #ddd;'></div>";*/
      var imgTitleAlt = "<div class='img-title-alt-box'>";
	  imgTitleAlt += "<label for='img-title' class='img-title-label note-form-label'>이미지 제목<i class='astc--mark'>*</i></label>";
	  imgTitleAlt += "<input id='img-title' class='img-title-form title-alt-form' type='text'/>";
	  imgTitleAlt +="<lable for='img-alt' class='img-alt-label note-form-label'>이미지 설명<i class='astc--mark'>*</i></label>";
	  imgTitleAlt += "<input for='img-alt' class='img-alt-form title-alt-form' type='text' />";
	  imgTitleAlt +="</div>";
	  var body = ['<div class="form-group note-form-group note-group-select-from-files">', '<label for="note-dialog-image-file-' + this.options.id + '" class="note-form-label">' + this.lang.image.selectFromFiles + '</label>', '<input id="note-dialog-image-file-' + this.options.id + '" class="note-image-input form-control-file note-form-control note-input" ', ' type="file" name="files" accept="' + this.options.acceptImageFileTypes + '" "/>', imageLimitation, '</div>','<div id="fileUploadContainer">','</div>', '<div class="form-group note-group-image-url">', '<label for="note-dialog-image-url-' + this.options.id + '" class="note-form-label">' + this.lang.image.url + '</label>', '<input id="note-dialog-image-url-' + this.options.id + '" class="note-image-url form-control note-form-control note-input" type="text"/>', '</div>'].join('')/*+imgBorder*/+imgTitleAlt;
      var buttonClass = 'btn btn-primary note-btn note-btn-primary note-image-btn';
      var footer = "<input type=\"button\" href=\"#\" class=\"".concat(buttonClass, "\" value=\"").concat("저장", "\" disabled>");
	...
	this.$dialog.find(".note-modal-content").addClass("note-image-box");
    }
    ...
    value: function showImageDialog() {
      ...
      return external_jQuery_default().Deferred(function (deferred) {
        ...
        _this2.ui.onDialogShown(_this2.$dialog, function () {
		  ...
		  var imageInputClone = $imageInput.clone();
		  var uploadSn = 0;
		  var fileNameList = [];
          $imageInput.replaceWith(imageInputClone.on('change', function (event) {
			$("#fileUploadContainer").empty();
			var uploadHtml = "<div style='margin:5px 0 0 10px;' id='uploadBox-"+uploadSn+"' >";
			uploadHtml+="<span style='font-weight:bold;color:#868686;'>";
			uploadHtml+=event.target.files[0].name;
			uploadHtml+="</span>";
			uploadHtml+="<button style='margin-left:8px;color:#564b4b;font-size:14px;' type='button' id='uploaddeleteBtn' onClick='fnuploadDelete("+uploadSn+");'>✖</button>";
			uploadHtml+="</div>";
			$("#fileUploadContainer").append(uploadHtml)
			uploadSn+=1
			$(".note-modal-footer input[value='저장']").removeAttr("disabled");
          }).val(''));

			$(".note-modal-footer input[value='저장']").on("click",function(){
				var imgtitle = $(".img-title-form").val();
			    var imgalt = $(".img-alt-form").val();
				if (imgtitle.trim() == '' || imgalt.trim() == ''){
			    		$(".note-image-input")[0].files[0].clear;
			    		alert("이미지 제목과 이미지 설명은 필수입니다.");
						$(".img-title-form").val('');
						$(".img-alt-form").val('');
						$("#fileUploadContainer").empty();
			    	}
				else{
			    	sendFile($(".note-image-input")[0].files[0],$("#summernote"),imgtitle,imgalt);	
		    		}
				
				$("#fileUploadContainer").empty();
				$("input[value='이미지']").attr("disabled","disabled");
				deferred.resolve($(".note-image-input")[0].files[0] || $(".note-image-input")[0].value);
			})
...
(external_jQuery_default()).summernote = external_jQuery_default().extend((external_jQuery_default()).summernote, {
  ...
  options: {
    ...
    icons: {
	  ...
	  'imgModify':'note-icon-img-modify',	
      'font': 'note-icon-font',
...
var ModalUI = /*#__PURE__*/function () {
  ...
  ModalUI_createClass(ModalUI, [{
    ...
    value: function show() {
	  ...
	  this.$modal.off('click', '.modal-close-btn').on('click', '.modal-close-btn', this.hide.bind(this));
